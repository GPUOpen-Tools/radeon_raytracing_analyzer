...
namespace rra
{
    ...
    public:
        ...
        std::stack<uint32_t> GetUserMarkerStringIndices(uint32_t sqttCbId, uint32_t userMarkerHistoryIndex) const;
    ...
    private:
        ...
        std::stack<uint32_t> RebuildUserMarkerStackUpUntil(const std::vector<MarkerInfo>& userMarkerHistory, uint32_t untilIdx) const;

        bool chunk_data_valid_ = false;  ///< Is the API info data valid.
        std::unordered_map<uint32_t, std::pair<uint32_t, std::vector<MarkerInfo>>>
            user_marker_history_map_;  ///< Contains pairs of (cb_id, (table_id, marker infos))
    };
