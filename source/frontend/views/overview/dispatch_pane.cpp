...
static const QColor kBackgroundColor = QColor(240, 240, 240);
...
{
    ui_->setupUi(this);
    rra::widget_util::SetWidgetBackgroundColor(this, kBackgroundColor);

    QPalette donut_palette = ui_->donut_->palette();
    donut_palette.setColor(QPalette::ColorRole::Window, kBackgroundColor);
    ui_->donut_->setPalette(donut_palette);
...
void DispatchPane::Update()
{
    ...
    std::stringstream dispatch_name_stream;
    dispatch_name_stream << dispatch_id_ << ": " << trace_rays_str;
    ...
    SetDispatchDimensions(dispatch_name_stream.str().c_str(), width, height, depth);

    // Repaint some elements.
    ui_->loading_bar_->repaint();
    ui_->donut_->repaint();

    ui_->dispatch_title_->setStyleSheet("ScaledPushButton#dispatch_title_ {font: 20pt; color: rgb(0, 0, 0); border: none; text-align: left;}");
    ...
    if (load_status.loading_complete && !load_status.has_errors)
    {
        ...
        ui_->dispatch_title_->setStyleSheet(
            "ScaledPushButton#dispatch_title_ {font: 20pt; color: rgb(0, 0, 255); border: none; text-align: left;} ScaledPushButton#dispatch_title_:hover "
            "{color: rgb(255, 128, 0);}");
