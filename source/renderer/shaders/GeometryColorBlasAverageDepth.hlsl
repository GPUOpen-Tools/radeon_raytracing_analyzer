...
/// @file
/// @brief  Shader for coloring by average depth.
//=============================================================================
...
struct VSInput
{
    ...
    [[vk::location(5)]] float  average_depth : METADATA0;
...
PSInput VSMain(VSInput input, uint vertexId : SV_VertexID)
{
    ...
    result.average_depth                    = input.average_depth;
...
float4 PSMain(PSInput input)
    : SV_TARGET
{
    float4 val                 = heatmap_temp(input.average_depth / (float)scene_ubo.max_tree_depth);
