...
/// @file
/// @brief  Shader for coloring by average SAH.
//=============================================================================
...
struct PSInput
{
    ...
    float  average_triangle_sah : METADATA0;
...
struct VSInput
{
    ...
    [[vk::location(5)]] float  average_triangle_sah : METADATA0;
...
PSInput VSMain(VSInput input, uint vertexId : SV_VertexID)
{
    ...
    result.average_triangle_sah             = input.average_triangle_sah;
...
float4 PSMain(PSInput input)
    : SV_TARGET
{
    float4 val                 = heatmap_temp(1.0 - input.average_triangle_sah);
