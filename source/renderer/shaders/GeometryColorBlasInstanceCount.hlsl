...
/// @file
/// @brief  Shader for coloring by instance count.
//=============================================================================
...
struct VSInput
{
    ...
    [[vk::location(5)]] uint   instance_count : METADATA0;
...
PSInput VSMain(VSInput input, uint vertexId : SV_VertexID)
{
    ...
    result.instance_count                   = input.instance_count;
...
float4 PSMain(PSInput input)
    : SV_TARGET
{
    float4 val                 = heatmap_temp((float)input.instance_count / (float)scene_ubo.max_instance_count);
