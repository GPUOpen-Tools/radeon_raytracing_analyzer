...
/// @file
/// @brief  Shader for coloring by minimum SAH.
//=============================================================================
...
struct PSInput
{
    ...
    float  min_triangle_sah : METADATA0;
...
struct VSInput
{
    ...
    [[vk::location(5)]] float  min_triangle_sah : METADATA0;
...
PSInput VSMain(VSInput input, uint vertexId : SV_VertexID)
{
    ...
    result.min_triangle_sah                 = input.min_triangle_sah;
...
float4 PSMain(PSInput input)
    : SV_TARGET
{
    float4 val                 = heatmap_temp(1.0 - input.min_triangle_sah);
