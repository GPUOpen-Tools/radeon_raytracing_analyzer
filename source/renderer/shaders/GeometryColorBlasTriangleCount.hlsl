...
/// @file
/// @brief  Shader for coloring by triangle count.
//=============================================================================
...
struct VSInput
{
    ...
    [[vk::location(5)]] uint   triangle_count : METADATA0;
...
PSInput VSMain(VSInput input, uint vertexId : SV_VertexID)
{
    ...
    result.triangle_count                   = input.triangle_count;
...
float4 PSMain(PSInput input)
    : SV_TARGET
{
    float4 val                 = heatmap_temp((float)input.triangle_count / (float)scene_ubo.max_triangle_count);
