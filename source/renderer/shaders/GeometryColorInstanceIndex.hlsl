...
/// @file
/// @brief  Shader for coloring by instance index.
//=============================================================================
...
struct PSInput
{
    ...
    int    instance_index : METADATA0;
    uint   blas_index : METADATA1;
...
struct VSInput
{
    ...
    [[vk::location(5)]] int    instance_index : METADATA0;
    [[vk::location(6)]] uint   blas_index : METADATA1;
...
PSInput VSMain(VSInput input, uint vertexId : SV_VertexID)
{
    ...
    result.instance_index                   = input.instance_index;
...
float4 PSMain(PSInput input)
    : SV_TARGET
{
    float  r                   = rand(asint(input.blas_index));
    float  g                   = rand(asint(input.instance_index) + asint(r));
    float  b                   = rand(asint(input.blas_index) + asint(g));
