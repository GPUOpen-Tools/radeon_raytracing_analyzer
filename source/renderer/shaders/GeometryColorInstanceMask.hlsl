...
/// @file
/// @brief  Shader for coloring by instance mask.
//=============================================================================
...
struct VSInput
{
    ...
    [[vk::location(6)]] uint   mask : METADATA1;
};
...
float4 PSMain(PSInput input)
    : SV_TARGET
{
    float  r                   = rand(input.mask);
    float  g                   = rand(input.mask + 1);
    float  b                   = rand(input.mask + 2);
    float4 coloring_mode_color = float4(r, g, b, 1.0);

    // If a mask is 0x00 we color the mask as black no matter what.
    coloring_mode_color = (input.mask == 0) ? float4(0.0, 0.0, 0.0, 0.0) : coloring_mode_color;
