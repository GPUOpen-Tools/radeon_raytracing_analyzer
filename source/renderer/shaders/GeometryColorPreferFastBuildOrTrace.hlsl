...
#define FAST_TRACE_BIT 0x4
#define FAST_BUILD_BIT 0x8
...
PSInput VSMain(VSInput input, uint vertexId : SV_VertexID)
{
    ...
    result.color = scene_ubo.build_algorithm_none_color;

    uint mask = FAST_TRACE_BIT | FAST_BUILD_BIT;
    if ((input.build_flags & mask) == mask)
    {
        result.color = scene_ubo.build_algorithm_both_color;
    }
    else if (input.build_flags & FAST_TRACE_BIT)
    {
        result.color = scene_ubo.build_algorithm_fast_trace_color;
    }
    else if (input.build_flags & FAST_BUILD_BIT)
    {
        result.color = scene_ubo.build_algorithm_fast_build_color;
    }
