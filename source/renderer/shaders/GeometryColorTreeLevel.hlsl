...
PSInput VSMain(VSInput input, uint vertexId : SV_VertexID)
{
    ...
    const uint rightmost_14_bits = 0x3FFF;

    result.position                         = mul(scene_ubo.view_projection, mul(input.instance_transform, float4(input.position, 1.0)));
    result.tree_depth                       = (input.geometry_index_depth_split_opaque >> 2) & rightmost_14_bits;  // Extract bits 15-2.
    ...
    : SV_TARGET
{
    ...
    coloring_mode_color = float4(color.rgb, 1.0f);
