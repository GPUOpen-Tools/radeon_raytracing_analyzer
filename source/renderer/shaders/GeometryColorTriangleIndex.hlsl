...
/// @file
/// @brief  Shader for coloring by geometry index.
//=============================================================================
...
struct VSInput
{
    ...
    [[vk::location(1)]] uint   triangle_index : METADATA0;
...
PSInput VSMain(VSInput input, uint vertexId : SV_VertexID)
{
    ...
    result.triangle_index                   = input.triangle_index;
...
float4 PSMain(PSInput input)
    : SV_TARGET
{
    float4 val                 = heatmap((float)input.triangle_index / 4.0f);
    float4 coloring_mode_color = float4(val.xyz, 1.0f);
