...
/// @file
/// @brief  Shader for coloring triangles by SAH.
//=============================================================================
...
struct VSInput
{
    ...
    [[vk::location(1)]] float  triangle_sah_and_selected : METADATA0;
...
PSInput VSMain(VSInput input, uint vertexId : SV_VertexID)
{
    ...
    result.sah                              = abs(input.triangle_sah_and_selected);
...
float4 PSMain(PSInput input)
    : SV_TARGET
{
    float4 val                 = heatmap_temp(1.0 - input.sah);
