...
/// @file
/// @brief  Shader for coloring by triangle splitting.
//=============================================================================
...
PSInput VSMain(VSInput input, uint vertexId : SV_VertexID)
{
    ...
    result.color                            = ((input.geometry_index_depth_split_opaque >> 1) & 1) ? scene_ubo.positive_color : scene_ubo.negative_color;
    ...
    : SV_TARGET
{
    ...
    {
        out_color = scene_ubo.selected_geometry_color;
    }
    ...
    return lerp(float4(input.wireframe_color, 1.0f), out_color, wireframe_alpha);
}
