...
/// @file
/// @brief  Color geometry by primitive structure.
//=============================================================================
...
struct PSInput
{
    ...
    int    primitive_structure_id : METADATA0;
...
struct VSInput
{
    ...
    [[vk::location(1)]] uint primitive_structure_id : METADATA0;
...
PSInput VSMain(VSInput input, uint vertexId : SV_VertexID)
{
    ...
    result.primitive_structure_id           = input.primitive_structure_id;
...
float4 PSMain(PSInput input)
    : SV_TARGET
{
    float r = rand(asint(input.primitive_structure_id));
    float g = rand(asint(input.primitive_structure_id) + asint(r));
    float b = rand(asint(input.primitive_structure_id) + asint(g));
